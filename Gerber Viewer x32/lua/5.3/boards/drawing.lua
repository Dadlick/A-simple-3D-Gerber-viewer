...
--- Create a simple rectangular aperture. This aperture unit is always `'pm'`, which is the unit of the *width* and *height*.
function _M.rectangle_aperture(width, height)
	local aperture = { shape = 'rectangle', unit = 'pm', width = width, height = height }
	return aperture
end
...
function _M.draw_text(image, polarity, fontname, size, mirror, halign, x, y, text)
	...
	text = {utf8.codepoint(text, 1, #text)}
	...
	elseif halign == 'x0' then
		local glyph = get_glyph(fontname, text[1])
