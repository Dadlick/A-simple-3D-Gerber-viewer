...
local function compile_expression(expression)
	...
	if t=='constant' then
		return math.tointeger(expression.value) or expression.value
...
local function apply_constraint(expression, dimension)
	...
	elseif t=='addition' or t=='subtraction' then
		...
		apply_constraint(expression[1], dimension)
		apply_constraint(expression[2], dimension)
	elseif t=='multiplication' then
		-- :KLUDGE: if one factor is a constant, apply the dimension to the other one
		if expression[1].type == 'constant' then
			apply_constraint(expression[1], {})
			apply_constraint(expression[2], dimension)
		elseif expression[2].type == 'constant' then
			apply_constraint(expression[1], dimension)
			apply_constraint(expression[2], {})
		end
		-- :TODO: use partial/relative dimensions
	elseif t=='division' then
		-- :KLUDGE: if the divisor is a constant, apply the dimension to the dividend
		if expression[2].type == 'constant' then
			apply_constraint(expression[1], dimension)
			apply_constraint(expression[2], {})
		end
		-- :TODO: use partial/relative dimensions
