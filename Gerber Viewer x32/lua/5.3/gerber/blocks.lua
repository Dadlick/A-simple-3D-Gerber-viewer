...
local function load_number(s, format)
	...
	if sign and integer and decimal then
		...
		elseif #decimal > format.decimal and decimal:sub(format.decimal + 1):match('^0+$') then
			-- ignore trailing zeroes
			decimal = decimal:sub(1, format.decimal)
		elseif #decimal > format.decimal then
			error("numbers with decimal dot and more significant decimal digits than the format are not yet supported")
		end
