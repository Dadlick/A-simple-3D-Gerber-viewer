...
--	WI = true, -- Window Specification
	TA = true, -- Metadata from Gerber X2 format
	TD = true, -- Metadata from Gerber X2 format
	TF = true, -- Metadata from Gerber X2 format
	TO = true, -- Metadata from Gerber X2 format
}
...
local function circle_center(x0, y0, i, j, x1, y1, direction, quadrant)
	...
	if quadrant=='single' then
		...
		for _,c in ipairs(centers) do
			...
			local ra = math.sqrt(dxa ^ 2 + dya ^ 2)
			local rb = math.sqrt(dxb ^ 2 + dyb ^ 2)
...
function _M.load(file_path)
	...
	for _,block in ipairs(data) do
		...
		elseif tb=='directive' then
			-- workaround for EasyEDA
			if block.G == 70 and block.D == 2 and not block.X and not block.Y and not block.I and not block.J and not block.M then
				block.D = nil
			end
			...
			elseif block.D or block.X or block.Y then
				...
				elseif movement == 'move' then
					-- move
					-- :TODO: if in region mode check that path is closed
					-- terminate current path if any
					path = nil
					if block.X then
						x = block.X * scale
			...
			elseif block.G==37 then
				region = false
				-- terminate current path if any
				-- :TODO: check that path is closed
				path = nil
...
function _M.save(image, file_path, verbose)
	...
	local region = false -- :TODO: generate consecutive regions inside a single G36/G37 pair
