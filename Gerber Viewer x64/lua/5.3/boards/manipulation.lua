...
function _M.scale_macro(macro, scale_x, scale_y)
	local copy = {
		name = macro.name,
		unit = macro.unit,
		script = macro.script,
...
--- 
function _M.scale_aperture(aperture, scale_x, scale_y, macros)
	local copy = {
		name = aperture.name,
		unit = aperture.unit,
		shape = aperture.shape,
	...
	if aperture.macro then
		copy.macro = macros[aperture.macro]
		if not copy.macro then
			copy.macro = _M.scale_macro(aperture.macro, scale_x, scale_y)
			macros[aperture.macro] = copy.macro
		end
	elseif aperture.shape=='circle' then
		assert(scale_y == scale_x or aperture.diameter == 0 and not aperture.hole_width and not aperture.hole_height, "circle non-uniform scaling is not yet supported")
		copy.diameter = aperture.diameter * scale_x
		copy.hole_width,copy.hole_height = scale_aperture_hole(aperture.hole_width, aperture.hole_height, scale_x)
	elseif aperture.shape=='rectangle' or aperture.shape=='obround' then
		assert(scale_y == scale_x, aperture.shape.." non-uniform scaling is not yet supported")
		copy.width,copy.height = scale_aperture_hole(aperture.width, aperture.height, scale_x)
		copy.hole_width,copy.hole_height = scale_aperture_hole(aperture.hole_width, aperture.hole_height, scale_x)
	elseif aperture.shape=='polygon' then
		assert(scale_y == scale_x, "polygon non-uniform scaling is not yet supported")
		copy.diameter = aperture.diameter * scale_x
		copy.steps = aperture.steps
		copy.hole_width,copy.hole_height = scale_aperture_hole(aperture.hole_width, aperture.hole_height, scale_x)
	elseif aperture.device then
...
--- 
function _M.scale_point(point, scale_x, scale_y)
	...
	-- fix x,y
	copy.x = point.x * scale_x
	copy.y = point.y * scale_y
	-- fix optional data
	if point.cx or point.cy then
		assert(scale_y == scale_x, "arc non-uniform scaling is not yet supported")
		copy.cx = point.cx * scale_x
		copy.cy = point.cy * scale_y
	end
	if point.x1 or point.y1 then
		copy.x1 = point.x1 * scale_x
		copy.y1 = point.y1 * scale_y
	end
	if point.x2 or point.y2 then
		copy.x2 = point.x2 * scale_x
		copy.y2 = point.y2 * scale_y
	end
	return copy
end

--- 
function _M.scale_path(path, scale_x, scale_y, apertures, macros)
	if not apertures then apertures = {} end
	if not macros then macros = {} end
	local copy = {
		unit = path.unit,
	...
	if path.aperture then
		copy.aperture = apertures[path.aperture]
		if not copy.aperture then
			copy.aperture = _M.scale_aperture(path.aperture, scale_x, scale_y, macros)
			apertures[path.aperture] = copy.aperture
		end
	end
	for i,point in ipairs(path) do
		copy[i] = _M.scale_point(point, scale_x, scale_y)
	end
	return copy
end

--- 
function _M.scale_layer(layer, scale_x, scale_y, apertures, macros)
	if not apertures then apertures = {} end
	if not macros then macros = {} end
	local copy = {
		polarity = layer.polarity,
	...
	for i,path in ipairs(layer) do
		copy[i] = _M.scale_path(path, scale_x, scale_y, apertures, macros)
	end
	return copy
end

--- 
function _M.scale_image(image, scale_x, scale_y, apertures, macros)
	if not apertures then apertures = {} end
	if not macros then macros = {} end
	local copy = {
		file_path = image.file_path,
	...
	-- scale layers
	for i,layer in ipairs(image.layers) do
		copy.layers[i] = _M.scale_layer(layer, scale_x, scale_y, apertures, macros)
	end
...
--- 
function _M.scale_outline(outline, scale_x, scale_y, apertures, macros)
	if not apertures then apertures = {} end
	if not macros then macros = {} end
	local copy = {
		apertures = {},
	...
	assert(not outline.path.aperture)
	copy.path = _M.scale_path(outline.path, scale_x, scale_y)
	
	-- scale apertures
	for type,aperture in pairs(outline.apertures) do
		copy.apertures[type] = apertures[aperture]
		if not copy.apertures[type] then
			copy.apertures[type] = _M.scale_aperture(aperture, scale_x, scale_y, macros)
			apertures[aperture] = copy.apertures[type]
		end
	end
...
--- 
function _M.scale_board(board, scale_x, scale_y)
	local copy = {
		unit = board.unit,
		template = board.template,
		extensions = {},
	...
	-- scale images
	for type,image in pairs(board.images) do
		copy.images[type] = _M.scale_image(image, scale_x, scale_y, apertures, macros)
	end
	
	-- scale outline
	if board.outline then
		copy.outline = _M.scale_outline(board.outline, scale_x, scale_y, apertures, macros)
	end
...
function _M.skew_macro(macro, s)
	local copy = {
		name = macro.name,
		unit = macro.unit,
		script = macro.script,
	}
	error("macro skewing not yet implemented")
	return copy
end

--- 
function _M.skew_aperture(aperture, skew_x, skew_y, macros)
	local copy = {
		name = aperture.name,
		unit = aperture.unit,
		shape = aperture.shape,
	...
	if aperture.macro then
		copy.macro = macros[aperture.macro]
		if not copy.macro then
			copy.macro = _M.skew_macro(aperture.macro, skew_x, skew_y)
			macros[aperture.macro] = copy.macro
		end
	elseif aperture.shape=='circle' and aperture.diameter == 0 and (copy.hole_width or 0) == 0 and (copy.hole_height or 0) == 0 then
		copy.diameter = aperture.diameter
		copy.hole_width = aperture.hole_width
		copy.hole_height = aperture.hole_height
	elseif aperture.shape=='circle' or aperture.shape=='rectangle' or aperture.shape=='obround' or aperture.shape=='polygon' then
		error(aperture.shape.." aperture skewing is not yet supported")
	elseif aperture.device then
		error("device skewing is not yet supported")
	else
		error("unsupported aperture shape")
	end
	return copy
...
--- 
function _M.skew_point(point, skew_x, skew_y)
	assert(point.x and point.y, "only points with x and y can be skewd")
	local copy = {}
	for k,v in pairs(point) do
		copy[k] = v
	end
	-- fix x,y
	copy.x = point.x + skew_x * point.y
	copy.y = point.y + skew_y * point.x
	-- fix optional data
	if point.cx or point.cy then
		error("arc skewing is not yet supported")
		copy.cx = point.cx * skew
		copy.cy = point.cy * skew
	end
	if point.x1 or point.y1 then
		copy.x1 = point.x1 + skew_x * point.y1
		copy.y1 = point.y1 + skew_y * point.x1
	end
	if point.x2 or point.y2 then
		copy.x2 = point.x2 + skew_x * point.y2
		copy.y2 = point.y2 + skew_y * point.x2
	end
	return copy
end

--- 
function _M.skew_path(path, skew_x, skew_y, apertures, macros)
	if not apertures then apertures = {} end
	if not macros then macros = {} end
	local copy = {
		unit = path.unit,
	...
	if path.aperture then
		copy.aperture = apertures[path.aperture]
		if not copy.aperture then
			copy.aperture = _M.skew_aperture(path.aperture, skew_x, skew_y, macros)
			apertures[path.aperture] = copy.aperture
		end
	end
	for i,point in ipairs(path) do
		copy[i] = _M.skew_point(point, skew_x, skew_y)
	end
	return copy
end

--- 
function _M.skew_layer(layer, skew_x, skew_y, apertures, macros)
	if not apertures then apertures = {} end
	if not macros then macros = {} end
	local copy = {
		polarity = layer.polarity,
	...
	for i,path in ipairs(layer) do
		copy[i] = _M.skew_path(path, skew_x, skew_y, apertures, macros)
	end
	return copy
end

--- 
function _M.skew_image(image, skew_x, skew_y, apertures, macros)
	if not apertures then apertures = {} end
	if not macros then macros = {} end
	local copy = {
		file_path = image.file_path,
	...
	-- skew layers
	for i,layer in ipairs(image.layers) do
		copy.layers[i] = _M.skew_layer(layer, skew_x, skew_y, apertures, macros)
	end
...
--- 
function _M.skew_outline(outline, skew_x, skew_y, apertures, macros)
	if not apertures then apertures = {} end
	if not macros then macros = {} end
	local copy = {
		apertures = {},
	...
	-- skew path (which should be a region)
	assert(not outline.path.aperture)
	copy.path = _M.skew_path(outline.path, skew_x, skew_y)
	
	-- skew apertures
	for type,aperture in pairs(outline.apertures) do
		copy.apertures[type] = apertures[aperture]
		if not copy.apertures[type] then
			copy.apertures[type] = _M.skew_aperture(aperture, skew_x, skew_y, macros)
			apertures[aperture] = copy.apertures[type]
		end
	end
...
--- 
function _M.skew_board(board, skew_x, skew_y)
	local copy = {
		unit = board.unit,
		template = board.template,
		extensions = {},
	...
	-- skew images
	for type,image in pairs(board.images) do
		copy.images[type] = _M.skew_image(image, skew_x, skew_y, apertures, macros)
	end
	
	-- skew outline
	if board.outline then
		copy.outline = _M.skew_outline(board.outline, skew_x, skew_y, apertures, macros)
	end
